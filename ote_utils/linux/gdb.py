...
class Gdb(LinuxAPI):
    ...
    def send_command_to_gdb(self, command='thread apply all bt'):
        ...
        for stripped_core_file, executable_file in Gdb.core_exec_map.items():
            command_string = ("sudo gdb -quiet -batch -ex '{cmd}' {exe} "
                              "{core}".format(
                                  cmd=command,
                                  exe=executable_file,
                                  core=stripped_core_file))
    ...
    def _find_core_files(self):
        ...
        for core_file_pattern in Gdb.CORE_PATTERNS:
            ...
            if Gdb.INCOMPLETE_CORE_PATTERN == core_file_pattern and files:
                raise GdbException('core incomplete something went wrong or increase timeout from: '
                                  + str(Gdb.DEFAULT_CORE_TIMEOUT) + ' seconds')
            else:
                core_files += files
        ...
        if len(all_files) != len(core_files):
            raise GdbException('Unknown files found in coredump folder '
                              + str(unknown_files))
        return core_files
