...
"""
***Not To Be Released to Third Party***
...
def parse_show_version(output, execution_time):
    ...
    Args:
        ...
        execution_time (float): time taken to collect output

    Returns:
        dict: formatted dict containing elements of command output
    """
    show_list = []
...
def parse_show_software(output, execution_time):
    """
    Parses 128T software info into standard dict

    Args:
        output (str): Scraped pcli command dump
        execution_time (float): time take to collect output

    Returns:
        dict: formatted dict containing elements of command output
              Multi-line output for a node is represented as lists of available versions
    """
    show_list = []
    dictionary, messages = parse_show_command(output, execution_time)

    current_node = []
    column_names = ["Available", "Downloading", "Downloaded"]
    for data_line in dictionary["show"]:
        if data_line["Node"] != "":
            if current_node != []:
                show_list.append(current_node)
            current_node = data_line.copy()
            for column in column_names:
                current_node[column] = []
        for column in column_names:
            if data_line[column] != "":
                current_node[column].append(data_line[column])
    show_list.append(current_node)
...
def parse_show_command(output, execution_time):
    ...
    *I95-11259, doesn't include all support
    ...
    """
    logger.debug("Parsing output received: %s", output)
    output, dictionary = _prep_show_output(output, execution_time)
    table_title, output = _strip_table_title(output)
    tables_list = _get_tables_list(output)
    show_list, messages = _parse_show_into_dictionary(tables_list)
    dictionary.update({"show": show_list})
...
def _clean_show_ouput(output):
    ...
    output = _strip_line_containing_match(output, "Since:")
    return output
...
def _parse_show_into_dictionary(tables_list):
    ...
    for table in tables_list:
        ...
        node, table = _strip_node(table)
        message = None
        delimiter_line, delimiter_index = _get_delimiter_line(table)
        if delimiter_line is None:
            break
        if delimiter_index != 0:
            message, table = _strip_message(table, delimiter_index)
        if message is not None:
            messages.append(message)
        columns = delimiter_line.split()
        spaces = re.findall(r"\s+", delimiter_line)
        if len(spaces) == 0:
            table_separator_length = 0
        else:
            table_separator_length = len(spaces[0])
        values, headers = _divide_output(table, columns, table_separator_length)
    ...
    logger.debug(f"show_dictionary_list is as follows: {show_dictionary_list}")
    return show_dictionary_list, messages


def _strip_message(table, delimiter_index):
    table_lines = table.split("\n")
    message = "\n".join(table_lines[0:delimiter_index])
    table = "\n".join(table_lines[delimiter_index:])
...
def _get_delimiter_line(output):
    ...
    for item in output_lines:
        if "==" in item:
            return item, output_lines.index(item)
    return None, None
...
def _strip_completion_time(output):
    completion_time = re.search("Completed in .* seconds", output).group(0)
    logger.debug("Located completion line: %s", completion_time)
    output = output.split(completion_time)[0].strip()
    completion_time = completion_time.split("Completed in ")[1]
    return {"completion_time": completion_time}, output


def _strip_table_title(output):
    line_index = [
        output.splitlines().index(delimiter)
        for delimiter in output.splitlines()
        if "---" in delimiter
    ]
    if line_index:
